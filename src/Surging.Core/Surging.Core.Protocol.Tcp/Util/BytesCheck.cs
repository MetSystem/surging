...
namespace Surging.Core.Protocol.Tcp.Util
{
    ...
    public class BytesCheck
    {
        ...
        /// <param name="Poly">多项式 默认0xA001</param>
        /// <param name="IsHighBefore">高位是否在前 是</param>
        /// <returns></returns>
        public static byte[] CRC16_Checkout(byte[] data, ushort Poly = 0xA001, bool IsHighBefore = true)
        {
            int i, j, crc_reg, check;
            crc_reg = (ushort)0xFFFF;   //CRC16 校验寄存器赋值为 0xFFFF
            ...
            byte[] Check = BitConverter.GetBytes((ushort)crc_reg);
            if (IsHighBefore)
            {
                return new byte[2] { Check[1], Check[0] };
            }
